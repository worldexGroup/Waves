...
class AddressActor(
  ...
  private def reserve(limitOrder: LimitOrder): Unit =
    for ((id, b) <- limitOrder.requiredBalance if b != 0) {
      log.trace(s"[$owner]${limitOrder.order.id()}: $id -> +$b ()")
      openVolume += id -> (openVolume(id) + b)
    }
  ...
  private def tradableBalance(assetId: Option[AssetId]): Long = portfolio.balanceOf(assetId) - openVolume(assetId)
  ...
  private def handleExecutionEvents: Receive = {
    case OrderCanceled(lo, _) =>
      log.trace(s"OrderCanceled(${lo.order.id()})")
      release(lo.order.id())
      activeOrders -= lo.order.id()
    case OrderAdded(submitted) =>
    ...
    case OrderExecuted(submitted, counter) =>
      ...
      if (counter.order.sender.toAddress == owner) {
        release(submitted.order.id())
        activeOrders += counter.order.id() -> counter
      }
  }
